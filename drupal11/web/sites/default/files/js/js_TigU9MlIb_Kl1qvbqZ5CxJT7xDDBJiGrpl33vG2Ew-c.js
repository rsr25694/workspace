...
/* @license GPL-2.0-or-later https://www.drupal.org/licensing/faq */
(function($,Drupal){Drupal.behaviors.domainAccessSettingsSummaries={attach(){if(typeof $.fn.drupalSetSummary==='undefined')return;function checkboxesSummary(context){const checkboxLabels=Array.from(context.querySelectorAll('input[type="checkbox"]:checked + label'));const values=checkboxLabels.map((label)=>label.textContent);if(values.length===0)values.push(Drupal.t('Not restricted'));return values.join(', ');}$('[data-drupal-selector="edit-visibility-domain-access"]').drupalSetSummary(checkboxesSummary);}};})(jQuery,Drupal);;
