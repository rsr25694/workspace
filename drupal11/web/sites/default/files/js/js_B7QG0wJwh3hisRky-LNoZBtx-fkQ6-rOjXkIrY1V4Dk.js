...
(($)=>{Drupal.behaviors.searchApiProcessor={attach(context){const selector='.search-api-status-wrapper input.form-checkbox';$(selector,context).each(function foreach(){const checkbox=this;const $checkbox=$(checkbox);const processorId=$checkbox.data('id');const $rows=$(`.search-api-processor-weight--${processorId}`,context);const tab=$(`.search-api-processor-settings-${processorId}`,context).data('verticalTab');$checkbox.on('click.searchApiUpdate',()=>{if(checkbox.matches(':checked')){$rows.show();if(tab)tab.tabShow().updateSummary();}else{$rows.hide();if(tab)tab.tabHide().updateSummary();}});if(tab)tab.details.drupalSetSummary(()=>{return checkbox.matches(':checked')?Drupal.t('Enabled'):Drupal.t('Disabled');});$checkbox.triggerHandler('click.searchApiUpdate');});}};})(jQuery);;
