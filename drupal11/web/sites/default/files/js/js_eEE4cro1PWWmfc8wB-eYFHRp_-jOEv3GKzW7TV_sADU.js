...
/* @license GPL-2.0-or-later https://www.drupal.org/licensing/faq */
(function(Drupal,document){Drupal.behaviors.domainSourceAllowed={attach(){const select=document.getElementById('edit-field-domain-source');if(!select)return;const initialOption=select.value;function setOptions(domains){const options=select.querySelectorAll('option');options.forEach((option)=>{if(option.value!=='_none'&&!domains.includes(option.value)){if(select.value===option.value)select.value='_none';option.hidden=true;}else{option.hidden=false;if(option.value===initialOption)select.value=option.value;}});}function getDomains(){const domains=[];const checked=document.querySelectorAll('#edit-field-domain-access input:checked');checked.forEach((input)=>{domains.push(input.value);});setOptions(domains);}getDomains();const accessContainer=document.getElementById('edit-field-domain-access');if(accessContainer)accessContainer.addEventListener('change',getDomains);}};})(Drupal,document);;
