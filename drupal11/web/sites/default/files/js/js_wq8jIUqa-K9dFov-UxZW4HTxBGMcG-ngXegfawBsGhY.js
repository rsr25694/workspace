...
(function($){'use strict';Drupal.behaviors.persistedQueries={attach:function(context,settings){$('.persisted-queries-enabled-wrapper input.form-checkbox',context).each(function(){var $checkbox=$(this);var plugin_id=$checkbox.data('id');var $rows=$('.persisted-queries-weight--'+plugin_id,context);var tab=$('.persisted-queries-settings--'+plugin_id,context).data('verticalTab');$checkbox.on('click.persistedQueryUpdate',function(){if($checkbox.is(':checked')){$rows.show();if(tab)tab.tabShow().updateSummary();}else{$rows.hide();if(tab)tab.tabHide().updateSummary();}});$checkbox.triggerHandler('click.persistedQueryUpdate');});}};})(jQuery);;
