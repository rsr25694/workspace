 ...
 * A CSP Header.
 *
 * @phpstan-type DirectiveValue (bool|string[])
 * @phpstan-type DirectiveSet array<string, DirectiveValue>
 */
class Csp {
  ...
  const DIRECTIVE_SCHEMA_TRUSTED_TYPES = 'serialized-trusted-types-configuration';
  const DIRECTIVE_SCHEMA_TRUSTED_TYPES_SINK_GROUPS = 'trusted-types-sink-groups';
  ...
  const DIRECTIVES = [
    ...
    // https://w3c.github.io/trusted-types/dist/spec/#trusted-types-csp-directive
    'trusted-types' => self::DIRECTIVE_SCHEMA_TRUSTED_TYPES,
    'require-trusted-types-for' => self::DIRECTIVE_SCHEMA_TRUSTED_TYPES_SINK_GROUPS,
   ...
   */
  public static function calculateHash(string $data, string $algorithm = 'sha256'): string {
   ...
   */
  public static function getDirectiveFallbackList(string $name): array {
    self::validateDirectiveName($name);
   ...
   */
  public function reportOnly(bool $value = TRUE): void {
   ...
   *
   * A default value will be returned based on the directive's schema if it has
   * not been set.
   * Use `hasDirective() == FALSE` to determine if the directive was not set.
   *
   * @param string $name
   *   The directive name.
   *
   ...
   */
  public function getDirective(string $name): array|bool {
    self::validateDirectiveName($name);

    $value = $this->directives[$name] ?? match(self::DIRECTIVES[$name]) {
      self::DIRECTIVE_SCHEMA_BOOLEAN => FALSE,
      default => [],
    };
   ...
   * @param string $name
   *   The directive name.
   * @param DirectiveValue|string $value
   *   An array of values, a boolean value, or a string that will be split on
   *   whitespace to separate values.
   */
  public function setDirective(string $name, array|bool|string $value): void {
    self::validateDirectiveName($name);
    ...
    elseif (is_bool($value)) {
      throw new \InvalidArgumentException("Cannot set '$name' to boolean value");
    }
   ...
   *   An array of values, or a string that will be split on whitespace to
   *   separate values.
   */
  public function appendDirective(string $name, array|string $value): void {
    self::validateDirectiveName($name);

    // Boolean directives cannot be appended.
    assert(self::DIRECTIVES[$name] !== self::DIRECTIVE_SCHEMA_BOOLEAN);
   ...
   *   separate values.
   */
  public function fallbackAwareAppendIfEnabled(string $name, array|string $value): void {
    self::validateDirectiveName($name);
  ...
  public function getHeaderValue(): string {
    ...
    foreach ($this->directives as $name => $value) {
      ...
      elseif (self::DIRECTIVES[$name] === self::DIRECTIVE_SCHEMA_TRUSTED_TYPES_SINK_GROUPS) {
        // Script is currently the only valid value.
  ...
  private static function reduceSourceList(array $sources): array {
    ...
    // Remove 'none' if ignored due to presence of other sources.
    if (
      in_array(Csp::POLICY_NONE, $sources)
      && !empty(array_diff($sources, [Csp::POLICY_NONE, Csp::POLICY_REPORT_SAMPLE]))
    ) {
      $sources = array_diff($sources, [Csp::POLICY_NONE]);
    }
   ...
   */
  private static function reduceAttrSourceList(array $sources): array {
   ...
   *
   * @param DirectiveSet $directives
   *   An array of directives.
   *
   * @return DirectiveSet
   *   The sorted directives.
   */
  public static function sortDirectives(array $directives): array {
   ...
   */
  private static function ff1313937(array $directives): array {
