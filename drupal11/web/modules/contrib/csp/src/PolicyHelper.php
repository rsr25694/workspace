...
class PolicyHelper {
   ...
   *   NULL to use the value from the nonce service (recommended), or a custom
   *   nonce value.
   */
  public function appendNonce(Csp $policy, string $directive, array|string $fallback, ?string $value = NULL): void {
    ...
    if (is_null($value)) {
      ...
      // Nonce should be at least 16 bytes (22 Base64 characters).
      if (!preg_match("<^'?nonce-([-A-Za-z0-9+/_]{22,}={0,2})'?$>i", $value, $matches)) {
        throw new \InvalidArgumentException("Value must be in the format \"'nonce-{base64-value}'\"");
      }
      $value = "'nonce-" . $matches[1] . "'";
    }
